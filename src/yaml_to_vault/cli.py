...
def _build(
    files: list[Path],
) -> tuple[list, VaultClient, str, str]:
    ...
    return plans, vault_client, inputs.env_name, inputs.environment.vault.mount
...
def plan_cmd(
    ...
    console = Console()
    try:
        plans, _, env_name, mount = _build(files)
    except (ConfigError, OnePasswordError, VaultClientError) as exc:
...
def apply_cmd(
    ...
    console = Console()
    try:
        plans, vault_client, env_name, mount = _build(files)
    except (ConfigError, OnePasswordError, VaultClientError) as exc:
        console.print(f"[red]{exc}[/red]")
        raise typer.Exit(code=1) from exc
